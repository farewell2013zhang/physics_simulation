"""
CEM-style minimal pipeline for Humanoid:
...
class EpisodeReplay:
    ...
    def split_train_test(self):
        train_cutoff = int(len(self.episodes) * 0.5)
        episodes = list(self.episodes)
        self.episodes_train = episodes[:train_cutoff]
        self.episodes_test = episodes[train_cutoff:]
    def sample_chunk(self, chunk_len, batch_size, mode='train'):
        """
        ...
        for i in range(batch_size):
            if mode == 'train':
                ep = random.choice(self.episodes_train)
            elif mode == 'test':
                ep = random.choice(self.episodes_test)
            L = ep['obs'].shape[0]
            if L <= chunk_len + 1:
                continue
    ...
    for it in pbar:
        ...
        if (it+1) % validate_every == 0:
            ...
            for i in range(30):
                ...
                obs_b, act_b = replay.sample_chunk(chunk_len=k_step, batch_size=batch_size*16, mode='test')
                xy = obs_b[:, [0], :2].copy()
                obs_b[:, :, :2] -= xy
                obs_b = np.clip(obs_b, -100, 100)
                with torch.no_grad():
                    ...
                    z_pred = z0
                    obs_rec = model.decode(z_pred)
                    loss_rec = F.mse_loss(obs_rec[:, [0,2]], obs0[:, [0,2]])
                    loss_rec_total = loss_rec_total + loss_rec
                    for t in range(k_step):
                        a_t = torch.tensor(act_b[:, t, :], dtype=torch.float32, device=device)
                        z_pred = model.step(z_pred, a_t)
                        obs_true = torch.tensor(obs_b[:, t+1, :], dtype=torch.float32, device=device)
                        obs_rec = model.decode(z_pred)
                        loss_rec = F.mse_loss(obs_rec[:, [0,2]], obs_true[:, [0,2]])
                        loss_rec_total = loss_rec_total + loss_rec
                loss += loss_rec_total / (k_step + 1) / 30
...
def main():
    ...
    collect_random_data(env, replay, obs_rms, model_ppo, min_steps=80000)
    # 2) train latent model
    print("Start training latent model...")
    save_model_path = "./humanoid_log/Stand_Walk/CEM"
    os.makedirs(save_model_path, exist_ok=True)
    latest_run_id = get_latest_run_id(save_model_path, 'CEM')
    save_path = os.path.join(save_model_path, f"CEM_{latest_run_id+1}")
