...
class ReplayBufferC1(ReplayBuffer):
    ...
    def add(
        ...
        while len(self.idx_list) and (self.pos - self.idx_list[0][1]) % self.buffer_size == 49:
            ...
            if self.pos >= 99:
                score = min(self.next_observations[self.pos-99:self.pos+1, c, 0].max(), 1.4)
            else:
                score = min(max(self.next_observations[self.pos-99+self.buffer_size:, c, 0].max(),
                                self.next_observations[:self.pos+1, c, 0].max()), 1.4)
            new_ele_flag = False
    ...
    def sample(self, batch_size: int, env = None):
        ...
        if not self.optimize_memory_usage:
            ...
            return self._get_samples(batch_inds, env_indices, batch_inds2, env_indices2, env=env)
    ...
    def _get_samples(self, batch_inds, env_indices, batch_inds2, env_indices2,  env = None):
        t0 = time.time()
        if self.n_steps == 1:
            if self.optimize_memory_usage:
            ...
            data = (
                np.concat([self._normalize_obs(self.observations[batch_inds, env_indices, :], env), 
                           self._normalize_obs(self.observations2[batch_inds2, env_indices2, :], env)], axis=0),
                np.concat([self.actions[batch_inds, env_indices, :], self.actions2[batch_inds2, env_indices2, :]], axis=0),
                np.concat([next_obs, next_obs2], axis=0),
                # Only use dones that are not due to timeouts
                # deactivated by default (timeouts is initialized as an array of False)
                np.concat([(self.dones[batch_inds, env_indices] * (1 - self.timeouts[batch_inds, env_indices])).reshape(-1, 1),
                           np.zeros_like(env_indices2).reshape(-1, 1)], axis=0),
                np.concat([self._normalize_reward(self.rewards[batch_inds, env_indices].reshape(-1, 1), env), 
                           self._normalize_reward(self.rewards2[batch_inds2, env_indices2].reshape(-1, 1), env)], axis=0),
            )
...
class EarlyStopAndSaveBestCallback(BaseCallback):
    ...
    def _on_rollout_end(self):
        ...
        if self.warmup < 0:
            self.warmup += 1
            if self.verbose:
                print(f"warmup={20-self.warmup}/20, value={current_value:.4f}")
            return True

        # Save the best model at current stage
        if self.num_timesteps > self.nround * 1000000:
            self.nround += 1
            best_model_path = os.path.join(self.save_path, f"best_model_step{self.num_timesteps}.zip")
            self.model.save(best_model_path)
            if hasattr(self.model.get_env(), "save"):
                self.model.get_env().save(os.path.join(self.save_path, f"vec_normalize_step{self.num_timesteps}.pkl"))
            np.save(os.path.join(self.save_path, f"obs_init_step{self.num_timesteps}.npy"), self.model.env.venv.unwrapped.envs[0].obs_init)
...
def main():
    save_model_path = "./humanoid_log/Stand_Walk/Lay Down Position/MlpPolicy"
    ...
    model = SAC("MlpPolicy", env, learning_rate=LinearSchedule(3e-5, 3e-5, 0.1), buffer_size=int(5e5), replay_buffer_class=ReplayBufferC1,
                n_steps=1, batch_size=4096, gamma=0.995, ent_coef="auto_0.005", target_entropy=-8.5, policy_kwargs=policy_kwargs, verbose=1, 
...
def eval():
    ...
    env = VecNormalize.load("./humanoid_log/Stand_Walk/Lay Down Position/MlpPolicy/SAC_1/vec_normalize.pkl", env)
    env.training = False
    env.norm_reward = False
    obs_init = np.load("./humanoid_log/Stand_Walk/Lay Down Position/MlpPolicy/SAC_1/obs_init.npy")
    obs = env.reset()
    ...
    model = SAC.load("./humanoid_log/Stand_Walk/Lay Down Position/MlpPolicy/SAC_1/best_model.zip", env=env)
...
def save_gif():
    import imageio
    model_names = sorted([f for f in os.listdir("./humanoid_log/Stand_Walk/Lay Down Position/MlpPolicy/SAC_1") if f.startswith("best_model") and f.endswith(".zip")])
    for model_name in model_names:
        ...
        env = VecNormalize.load(f"./humanoid_log/Stand_Walk/Lay Down Position/MlpPolicy/SAC_1/vec_normalize{suffix}.pkl", env)
        env.training = False
        env.norm_reward = False
        obs_init = np.load(f"./humanoid_log/Stand_Walk/Lay Down Position/MlpPolicy/SAC_1/obs_init{suffix}.npy")
        obs = env.reset()
        env.venv.unwrapped.envs[0].data.qpos[2:] = obs_init[:22]
        env.venv.unwrapped.envs[0].data.qvel[:] = 0
        obs = obs_init.reshape(1,-1)
        ...
        # Load pretrained model
        model = SAC.load(f"./humanoid_log/Stand_Walk/Lay Down Position/MlpPolicy/SAC_1/{model_name}", env=env)
        ...
        a = sum(z_hist)/len(z_hist)
        print(f"average height: {a}")
        imageio.mimsave(f"./humanoid_log/Stand_Walk/Lay Down Position/MlpPolicy/SAC_1/{final}.gif", frames, fps=30)
        env.close()
