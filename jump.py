# curiosity_goalachiever_humanoid.py
...
class EnvWrapper(gym.Wrapper):
    ...
    def step(self, action):
        ...
        if obs[0] > 0.8:
            reward += (obs[0] - 0.8) * 1
        self.zmax = max(self.zmax, obs[0])
        if done or terminated:
            self.zmax_hist.append(self.zmax)
    ...
    def reset(self, **kwargs):
        ...
        # Reset position to crouch the knees and hips
        qpos[5] = 0.18  # torso z
        qpos[8] = 0.57  # abdomen z
        qpos[12] = -1.8  # left hip z
        qpos[13] = -1.5   # left knee
        qpos[16] = -1.8 # right hip z
        qpos[17] = -1.5  # right knee
...
class EarlyStopAndSaveBestCallback(BaseCallback):
    ...
    def _on_rollout_end(self):
        ...
        for env in self.model.get_env().venv.envs:
            zmax_hist.append(env.env.zmax_hist)
            env.zmax_hist = []
        zmax_hist = np.concat(zmax_hist)
        current_value = safe_mean(zmax_hist)
...
def make_env(render_mode=None):
    env = gym.make("Humanoid-v5", max_episode_steps=240, frame_skip=3,
                    ...
                    ctrl_cost_weight=0.1, healthy_reward=1,
                    healthy_z_range=(0.8, 1e10),
                    render_mode=render_mode)
    return Monitor(EnvWrapper(env))

def main():
    save_model_path = "./humanoid_log/Jump/MlpPolicy"
    ...
    model = PPO("MlpPolicy", venv, learning_rate=LinearSchedule(3e-5, 1e-5, 1), clip_range=0.2, n_epochs=5, vf_coef=1.5,
                n_steps=4096, batch_size=256, gamma=0.98, ent_coef=0.005, policy_kwargs=policy_kwargs, verbose=1, 
                target_kl=0.1, tensorboard_log=save_model_path)

    callback = EarlyStopAndSaveBestCallback(save_path, patience=10, min_delta=0, verbose=0)
...
def eval():
    ...
    venv = VecNormalize.load("./humanoid_log/Jump/MlpPolicy/PPO_1/vec_normalize.pkl", venv)
    venv.training = False
    venv.norm_reward = False
    obs = venv.reset()
    model = PPO.load("./humanoid_log/Jump/MlpPolicy/PPO_1/best_model", env=venv)
